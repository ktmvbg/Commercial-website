...
export default function OrderDetail() {
    ...
    const [products, setProducts] = useState([]); // [{id, name, description, href, price, status, step, date, datetime, address, email, phone, imageSrc, imageAlt}]
    ...
    useEffect(() => {
        ...
        const url = config.API + '/api/order/' + orderId;
        fetch(url,
            {
                headers: {
            ...
            .then(data => {
                ...
                for (const product of data.order_products) {
                    const tempProduct = {
                        ...
                        status: data.status == 4 ? "Delivered" : data.status == 3 ? 'Cancelled' : data.status == 2 ? 'Processing' : data.status == 1 ? 'Confirmed' : 'Preparing', // 'Preparing to ship', 'Shipped', 'Delivered', 'Cancelled
                        step: data.status - 1,
    ...
    if (loading || !order || !router.isReady) {
                                    ...
                                    <div className="px-4 py-6 sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:p-8">
                                        <div className="sm:flex lg:col-span-7">
                                            ...
                                            <div className="mt-6 sm:ml-6 sm:mt-0">
                                                ...
                                                <p className="mt-2 text-sm font-medium text-gray-900">{product.price.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</p>
                                                ...
                                                order.status == 3 ? (
                                                    <>
                                                        ...
                                                        <div className="mt-6 hidden grid-cols-2 text-sm font-medium text-gray-600 sm:grid">
                                                            ...
                                                            <div className={classNames('text-indigo-600', 'text-right')}>
                                                                Cancelled
                                                            </div>
                                                        </div>
                                                    </>
                                                    ...
                                                    <>
                                                        <div className="overflow-hidden rounded-full bg-gray-200">
                                                            <div
                                                                className="h-2 rounded-full bg-indigo-600"
                                                                style={{ width: `calc((${product.step == 3 ? 4 : product.step} * 2 + 1) / 8 * 100%)` }}
                                                            />
                                                        </div>
                        ...
                        <div className="bg-gray-100 px-4 py-6 sm:rounded-lg sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:px-8 lg:py-8">
                            ...
                            <dl className="mt-8 divide-y divide-gray-200 text-sm lg:col-span-5 lg:mt-0">
                                <div className="flex items-center justify-between pb-4">
                                    ...
                                    <dd className="font-medium text-gray-900">{order.total.toLocaleString('vi-VN', {style : 'currency', currency : 'VND'})}</dd>
                                </div>
                                <div className="flex items-center justify-between py-4">
                                    <dt className="text-gray-600">Shipping</dt>
                                    <dd className="font-medium text-gray-900">0 đ</dd>
                                </div>
                                <div className="flex items-center justify-between py-4">
                                ...
                                <div className="flex items-center justify-between pt-4">
                                    ...
                                    <dd className="font-medium text-indigo-600">{order.total.toLocaleString('vi-VN', {style : 'currency', currency : 'VND'})}</dd>
                                </div>
                            </dl>
                        </div>
                    </section>
